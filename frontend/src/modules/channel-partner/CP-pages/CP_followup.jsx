...
const CP_followup = () => {
  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      const followupLeads = leads.filter(lead => lead.status === 'followup' || (lead.followUps && lead.followUps.length > 0))
      setLeadsData(followupLeads)
    } catch (error) {
  ...
  const followUpItems = useMemo(() => {
    ...
    const items = leadsData.flatMap(lead => {
      ...
      return lead.followUps
        .filter(fu => {
          const fuDate = new Date(fu.scheduledDate || fu.date)
          fuDate.setHours(0, 0, 0, 0)
          return fuDate >= today && (fu.status === 'pending' || fu.status === 'completed')
        })
        .map(followUp => ({
          _id: followUp._id || `${lead._id}-${followUp.scheduledDate || followUp.date}`,
          followUpId: followUp._id,
          leadId: lead._id,
          scheduledDate: followUp.scheduledDate || followUp.date,
          scheduledTime: followUp.scheduledTime || followUp.time,
  ...
  const handleCompleteFollowUp = async (leadId, followUpId) => {
    try {
      ...
      await cpLeadService.updateFollowUp(leadId, followUpId, { status: 'completed' })
  ...
  const handleCancelFollowUp = async (leadId, followUpId) => {
    try {
      await cpLeadService.updateFollowUp(leadId, followUpId, { status: 'cancelled' })
  ...
  const handleFollowUpSubmit = async (followUpData) => {
    ...
    try {
      ...
      await cpLeadService.updateFollowUp(
